...
export default function ProfileScreen({ navigation }) {
  ...
  // ------------ IMAGE PICKER WITH CAMERA + GALLERY + DELETE ----------
  ...
  const takePhoto = async () => {
    ...
    if (!result.canceled) {
      ...
      if (!result.assets || !result.assets[0].base64) {
        console.log("Camera returned NO BASE64, manually converting...");
        return Alert.alert(
          "Error",
          "Failed to read image. Try again or pick from gallery."
        );
      }
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      console.log("No profile — creating new...");
      const newProfile = {
        email: user.email,
        name: user.name || "",
        phone: "",
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.name}>{formData.name || user?.name || "User"}</Text>
...
const styles = StyleSheet.create({
  ...
  statNumber: { fontSize: 24, fontWeight: 'bold', color: '#2b8a3e' },
